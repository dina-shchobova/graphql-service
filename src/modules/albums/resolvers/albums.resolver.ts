const albumsResolver = {
  ...
  Mutation: {
    createAlbum: (_, {albumInput}, {dataSources}) => {
      try {
        return dataSources.albumsService.createAlbum(albumInput);
      } catch (e) {
        return null;
      }
    },
    ...
    updateAlbum: (_, {id, updateAlbumInput}, {dataSources}) => {
      try {
        return dataSources.albumsService.updateAlbum(id, updateAlbumInput);
      } catch (e) {
        return null;
      }
    },
